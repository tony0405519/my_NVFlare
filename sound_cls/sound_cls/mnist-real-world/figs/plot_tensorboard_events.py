...
experiments = {
    "mnist_fedavg_stream_tb": {"tag": "val_acc_local_model", "alpha": 1.0},
    # "cifar10_fedavg_stream_tb": {"tag": "val_acc_local_model", "alpha": 1.0},
...
def find_job_id(workdir, fl_app_name="mnist_fedavg", alpha=None):
...
def main():
    ...
    for config, exp in experiments.items():
        ...
        if add_cross_site_val:
            ...
            for k in xsite_keys:
                try:
                    xsite_data[k].append(xsite_results["site-2"][k]["val_accuracy"])
