...
TRAIN_ROOT = "/root/datasets"
VALID_ROOT = "/workspace/NVFlare/cifar10/train_root/valid_data"
# MODEL_PATH = "/home/aienode1/NVFlare/sound_cls/train_root/model.pt"

class MNISTLearner(Learner):  # also supports CIFAR10ScaffoldLearner
    def __init__(
        self,
        train_idx_root: str = "",
    ...
    def initialize(self, parts: dict, fl_ctx: FLContext):
        ...
        print(f'************ get analytic sender id: {self.analytic_sender_id} *********************')
        ...
        self.model = AudioCRNN(10).to(self.device)
    ...
    def _create_datasets(self, fl_ctx=None):
        ...
        if self.train_dataset is None or self.train_loader is None:
            ...
            self.train_dataset = CSVDataManager(TRAIN_ROOT, train=True)
            self.train_loader = self.train_dataset.get_loader(self.transform_train, batch_size=self.batch_size, shuffle=True, num_workers=self.num_workers)

        if self.valid_dataset is None or self.valid_loader is None:
            self.valid_dataset = CSVDataManager(VALID_ROOT)
            self.valid_loader = self.valid_dataset.get_loader(self.transform_valid, batch_size=self.batch_size, shuffle=False, num_workers=self.num_workers)
    ...
    def train(self, shareable: Shareable, fl_ctx: FLContext, abort_signal: Signal) -> Shareable:
        ...
        if os.path.exists(MODEL_PATH):
            local_var_dict = torch.load(MODEL_PATH)['model_weights']
            print("---------------------------------------------Pretrained model loaded!!!!!!!!!!!!!!!!!!!!!")
        else:
            local_var_dict = self.model.state_dict()
            print("---------------------------------------------New model!!!!!!!!!!!!!!!!!!!!!")
        '''
        print('------------- [client] generate defalut weight --------------')
        local_var_dict = self.model.state_dict()
        ...
        for var_name in local_var_dict:
            if var_name in model_keys:
                ...
                try:
                    print('----------------------- [client] got weight from server !!!!!!!!!!!!!!')
    ...
    def local_valid(self, valid_loader, abort_signal: Signal, tb_id=None, fl_ctx=None):
        ...
        with torch.no_grad():
            correct, total = 0, 0
            for _i, batch in enumerate(valid_loader):
                if abort_signal.triggered:
                    return None
                inputs = list(batch[:-1])
                labels = batch[-1]
    ...
    def validate(self, shareable: Shareable, fl_ctx: FLContext, abort_signal: Signal) -> Shareable:
        ...
        if os.path.exists(MODEL_PATH):
            local_var_dict = torch.load(MODEL_PATH)['model_weights']
            print('--------------------------------------[validate] pretrained model loaded')
        else:
            local_var_dict = self.model.state_dict()
            print('--------------------------------------[validate] new model loaded')
        '''    
        print('---------- [client] pretrain model generate from client for validate !!!')
        local_var_dict = self.model.state_dict()
        # load model from server and cover local model
        model_keys = global_weights.keys()
        n_loaded = 0
        for var_name in local_var_dict:
            if var_name in model_keys:
                ...
                try:
                    # update the local dict
                    print('--------------------------- [client] get pretrained model from server for validation !!!!!')
